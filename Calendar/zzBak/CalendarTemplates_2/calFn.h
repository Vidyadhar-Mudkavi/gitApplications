...
class calFn
{
   ...
   // cycle of days
      static int KDayBefore(const calRataDie<int>& rd, int k);
      static int KDayOnOrBefore(const calRataDie<int>& rd, int k);

      static int KDayAfter(const calRataDie<int>& rd, int k);
      static int NthKDay(int n, int k, const calGregorian<int>& g_date);

      static int DayOfWeekFromFixed(const calRataDie<int>& rd);
...
inline long int calFn::FloorFracLongDouble(long double num, long double den)
{
...
inline long double calFn::ModLongDouble(long double x, long double y)
{
...
inline long int calFn::CeilFrac(long int num, long int den)
{
// first check if num is exactly divisible by den (do integer arithmetic)
   long int abs_num = (num < 0) ? -num : num;
   long int abs_den = (den < 0) ? -den : den;
...
// is there a case of "exactly" divisible? hard to say.
// will not do this here and now.
inline long int calFn::CeilFracLongDouble(long double num, long double den)
{
   long double frac = num/den;

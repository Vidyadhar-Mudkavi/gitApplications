...
const char* calRataDie_id = 
   "@(#) calRataDie. implementation. ver. 1.1. Premalatha, Vidyadhar Mudkavi, CTFD, NAL.";
// function prototypes
...
calGregorian calRataDie::Gregorian() const
{
   ...
   prior_days = date - g._RDValue();

// calculate the correction
   calDate march_first(year, 3, 1);
   calGregorian g_march( march_first );

   if ( date < g_march._RDValue() )
   {
      correction = 0;
   }
   else
   {
      if ( (date >= g_march._RDValue()) && (g_march.isLeapYear() == true) )
      {
         correction = 1;
      }
      else
...
calJulian calRataDie::Julian() const
{
   ...
   prior_days = date - j._RDValue();

// calculate the correction. to simulate 30-day feb
   calDate march_first(year, 3, 1);
   calJulian j_march( march_first );

   if ( date < j_march._RDValue() )
   {
      correction = 0;
   }
   else
   {
      if ( (date >= j_march._RDValue()) && (j_march.isLeapYear() == true) )
      {
         correction = 1;
      }
      else
...
calIso calRataDie::Iso() const
{
   ...
   iso = iso_date;
   week = calFn::FloorFrac( (date - iso._RDValue()), 7) + 1;

...
#endif
calGregorianWeekDay calRataDie::pv_weekDay;
...
/**
  ...
  */
zOHL calRataDie::OldHinduLunar() const
{
#if 1
   long double sun = static_cast<long double>( zOHL::DayCount(pv_date) ) +
                     static_cast<long double>( 0.25 );

   long double new_moon = sun - calFn::ModLongDouble(sun, zOHL::AryaLunarMonth());
   ...
   bool leap_month = (  ( new_moon_mod_solar_month > 0 ) &&
                        ( calOldHinduSolar::AryaSolarMonth() - zOHL::AryaLunarMonth() ) 
   ...
   long double ld_day = calFn::FloorFracLongDouble(sun, zOHL::AryaLunarDay());
   ...
   long int year  = calFn::CeilFracLongDouble( (new_moon + calOldHinduSolar::AryaSolarMonth()), 
    ...
    return zOHL(year, static_cast<int>(month), leap_month, static_cast<int>(day));

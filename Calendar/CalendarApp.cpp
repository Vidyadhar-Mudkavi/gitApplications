...
// begin application
   ...
   // calOldHinduLunar is an imitation written afresh to settle this problem.
      ...
      out << " Old hindu lunar: " << old_hl << " Gregorian: " << g << std::endl << std::endl;
      pm << plume::pause;
#if 1
...
#if 1 // data for Amogh to test his Java code
...
#if 1 // birth day request from ranebennur
      y = 1959;   m =  6;  d = 8;
      calDate birthDay(y, m, d);
      g = birthDay;
      out << "Day of week for: " << g << " is " << g.Fixed().DayOfWeek() << std::endl << std::endl;
      pm << plume::pause;
#endif
...
// this function tests the astonomical functions
//
void TestAstroFunctions()
{
   std::cout << " *** Testing astro functions *** " << std::endl;

   positionMarker pm;

// test timeZone
#if 1
   std::cout << " *** Test timeZone class ***" << std::endl;
   timeZone tz1(2);
   std::cout << tz1 << std::endl;

   timeZone tzIndia(5, 30);

   std::cout << tzIndia << std::endl;
...
#if 1
   std::cout << std::endl
             << " *** Test latitude, longitude, elevation and terrestrial location classes ***"
             << std::endl;
   aLatitude   latMecca;
   latMecca.Set(21, 25, 24);
   std::cout << std::endl
             << " *** location of Mecca: " << latMecca << std::endl;

   aLatitude latUrbana;
   latUrbana.Set( 40.1 );
   std::cout << std::endl
             << " *** location of Urbana: " << latUrbana << std::endl;

   aLatitude latJerusalem;
   latJerusalem.Set( 31.8 );
   std::cout << std::endl
             << " *** location of Jerusalem: " << latJerusalem << std::endl;

   pm << plume::pause;

   aLatitude latBangalore;
   latBangalore.Set(12, 58);  // lon: 77, 35
   std::cout << std::endl
             << " *** location of Bangalore: " << latBangalore << std::endl;
